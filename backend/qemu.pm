...
sub save_memory_dump ($self, $args) {
    ...
    log::diag 'Memory dump completed';
...
sub save_storage_drives ($self, $args) {
    log::diag "Attempting to extract disk #%d.", $args->{disk};
    ...
    log::diag "Successfully extracted disk #%d", $args->{disk};
    return;
}
...
sub save_snapshot ($self, $args) {
    ...
    $bdc->for_each_drive(sub {
            ...
            if ($rsp->{error}) {
                log::diag('blockdev-snapshot-sync(' . Dumper($req) . ') -> ' . Dumper($rsp));
...
sub start_qemu ($self) {
    ...
    if ($vars->{NICTYPE} eq "vde") {
        ...
        if ($vars->{VDE_USE_SLIRP}) {
            ...
            log::diag join(' ', @cmd) . " started with pid $child_pid";
...
sub is_shutdown ($self, @) {
    ...
    log::diag("QEMU status is not 'shutdown', it is '$ret'") if $ret ne 'shutdown';
