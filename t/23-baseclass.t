...
subtest 'adjusting pipe size for external video encoder ' => sub {
    ...
    my $stderr = stderr_from { ok $baseclass->_start_external_video_encoder_if_configured, 'video encoder started' };
    like $stderr, qr{Launching external video encoder}, 'message logged';
    my @video_encoder_pids = keys %$video_encoders;
    is scalar @video_encoder_pids, 1, 'one video encoder started';
    my $launched_video_encoder = $video_encoders->{$video_encoder_pids[0]};
    ...
    subtest 'pipe size set' => sub {
        if ($stderr =~ /Can't increase video encoder pipe size/) {
            ...
            ok $pipe_sz >= 640 * 480 * 3, 'pipe size set' or diag "Pipe size is $pipe_sz, expected >= " . (640 * 480 * 3);
        }
