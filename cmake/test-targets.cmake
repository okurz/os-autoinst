...
add_test(
    ...
    COMMAND make test-perl TESTS="xt/" PROVE=${PROVE_PATH}
    WORKING_DIRECTORY "${CMAKE_CURRENT_SOURCE_DIR}"
)
...
add_test(
    ...
    COMMAND make test-perl PROVE=${PROVE_PATH}
    WORKING_DIRECTORY "${CMAKE_CURRENT_SOURCE_DIR}"
)
...
add_custom_target(test-ctest COMMAND ${CMAKE_CTEST_COMMAND} -E "test-perl-testsuite|test-local-author-perl" USES_TERMINAL VERBATIM)
foreach (CUSTOM_TARGET test-local-author-perl test-perl-testsuite test-ctest check check-pkg-build)
...
if (COVER_PATH AND PROVE_PATH)
    add_custom_command(
        ...
        COMMAND rm -rf "${CMAKE_CURRENT_SOURCE_DIR}/cover_db"
        COMMAND make test-perl COVERAGE=1 PROVE=${PROVE_PATH}
        OUTPUT "${CMAKE_CURRENT_SOURCE_DIR}/cover_db"
        WORKING_DIRECTORY "${CMAKE_CURRENT_SOURCE_DIR}"
    )
    add_custom_command(
        COMMENT "Generate coverage report (HTML)"
        COMMAND "${COVER_PATH}" -report html_basic "${CMAKE_CURRENT_SOURCE_DIR}/cover_db" -outputdir "${CMAKE_CURRENT_SOURCE_DIR}" -ignore "^external/" -ignore "^tools/" -ignore "^t/data/" -ignore "^/tmp"
