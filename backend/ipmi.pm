...
sub new ($class) {
    $tiedvars::vars{WORKER_HOSTNAME} or die 'Need variable WORKER_HOSTNAME';
    return $class->SUPER::new;
}

sub ipmi_cmdline ($self) {
    $tiedvars::vars{"IPMI_$_"} or die 'Need variable IPMI_$_' foreach qw(HOSTNAME USER PASSWORD);
    return ('ipmitool', '-I', 'lanplus', '-H', $tiedvars::vars{IPMI_HOSTNAME}, '-U', $tiedvars::vars{IPMI_USER}, '-P', $tiedvars::vars{IPMI_PASSWORD});
}
...
sub dell_sleep ($self) {
    return unless ($tiedvars::vars{IPMI_HW} || '') eq 'dell';
    sleep 4;
}
...
sub do_start_vm ($self, @) {
    ...
    $self->do_mc_reset if $tiedvars::vars{IPMI_BACKEND_MC_RESET};
    $self->get_mc_status;
    $self->restart_host unless $tiedvars::vars{IPMI_DO_NOT_RESTART_HOST};
    $self->truncate_serial_file;
    my $sol = $testapi::distri->add_console(
        'sol', 'ipmi-xterm',
        {
            persistent => $tiedvars::vars{IPMI_SOL_PERSISTENT_CONSOLE} // 1,
...
sub do_stop_vm ($self, @) {
    $self->ipmitool("chassis power off") unless $tiedvars::vars{IPMI_DO_NOT_POWER_OFF};
...
sub get_mc_status ($self) {
    ...
    $self->ipmitool("mc selftest") unless $tiedvars::vars{IPMI_SKIP_SELFTEST};
}
...
sub do_mc_reset ($self) {
    ...
    my $max_tries = $tiedvars::vars{IPMI_MC_RESET_MAX_TRIES} // 5;
    for (1 .. $max_tries) {
        ...
        if (my $E = $@) {
            ...
            sleep $tiedvars::vars{IPMI_MC_RESET_SLEEP_TIME_S} // 10;
            bmwqemu::diag('sleep period ends, probing connection with ping');
            # check until  mc reset is done and ipmi recovered
            my $count = 0;
            my $timeout = $tiedvars::vars{IPMI_MC_RESET_TIMEOUT} // ONE_MINUTE;
            my $ping_count = $tiedvars::vars{IPMI_MC_RESET_PING_COUNT} // 1;
            my $ping_cmd = "ping -c$ping_count '$tiedvars::vars{IPMI_HOSTNAME}'";
            my $ipmi_tries = $tiedvars::vars{IPMI_MC_RESET_IPMI_TRIES} // 3;
            while ($count++ < $timeout) {
