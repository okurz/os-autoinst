...
sub new ($class) {
    ...
    defined $tiedvars::vars{HMC_MACHINE_NAME} or die 'Need variable HMC_MACHINE_NAME';
...
sub do_start_vm ($self, @) {
    ...
    my $ssh = $testapi::distri->add_console(
        ...
        'ssh-xterm',
        {
            hostname => $tiedvars::vars{HMC_HOSTNAME} || die('Need variable HMC_HOSTNAME'),
            password => $tiedvars::vars{HMC_PASSWORD} || die('Need variable HMC_PASSWORD'),
            username => $tiedvars::vars{HMC_USERNAME} // 'hscroot',
            persistent => 1,
...
sub run_cmd ($self, $cmd, $hostname = undef, $password = undef, @) {
    $hostname //= $tiedvars::vars{HMC_HOSTNAME} or die 'Need variable HMC_HOSTNAME';
    $password //= $tiedvars::vars{HMC_PASSWORD} or die 'Need variable HMC_PASSWORD';
    my $username = $tiedvars::vars{HMC_USERNAME} // 'hscroot';
...
sub is_shutdown ($self, @) {
    my $lpar_id = $tiedvars::vars{LPAR_ID} or die 'Need variable LPAR_ID';
    my $hmc_machine_name = $tiedvars::vars{HMC_MACHINE_NAME} or die 'Need variable HMC_MACHINE_NAME';
