...
sub log_format_callback { log::log_format_callback(@_) }
sub diag { log::diag(@_) }

sub fctres {
    my ($text, $fname) = @_;
    $fname //= (caller(1))[3];
    log::fctres($text, $fname);
}

sub fctinfo {
    my ($text, $fname) = @_;
    $fname //= (caller(1))[3];
    log::fctinfo($text, $fname);
}

sub fctwarn {
    my ($text, $fname) = @_;
    $fname //= (caller(1))[3];
    log::fctwarn($text, $fname);
}

sub modstate { log::modstate(@_) }
sub init_logger { log::init_logger(@_) }
